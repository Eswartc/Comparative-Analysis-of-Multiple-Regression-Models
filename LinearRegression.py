...
# model.score(x_train, y_train) = 0.9571105088024204

#Using the Testing data to predict
predicted = model.predict(x_test)

plt.scatter(predicted, y_test, color='blue', label='Data Points')
...
MAE = mae(y_test, predicted)                                     # MAE = 5261.1495083836235
MSE = mse(y_test, predicted)                                     # MSE = 33026775.94619207
RMSE = np.sqrt(MSE)                                              # RMSE = 5746.892720957307
R2 = r2_score(y_test, predicted)                                 # R2_SCORE = 0.9827789020862662
MPE = np.mean((-y_test + predicted)/y_test)*100                  # MPE = 1.9795079930294828
